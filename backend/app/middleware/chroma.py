...
连接参数与集合名统一来自 database_settings（数据库配置中心）。
"""
...
class ChromaClient:
    ...
    def connect(cls) -> None:
        ...
        cls._ensure_database(
            database_settings.CHROMA_HOST,
            database_settings.CHROMA_PORT,
            database_settings.CHROMA_TENANT,
            database_settings.CHROMA_DATABASE,
        )
        cls.client = chromadb.HttpClient(
            host=database_settings.CHROMA_HOST,
            port=database_settings.CHROMA_PORT,
            tenant=database_settings.CHROMA_TENANT,      # 项目共用 default_tenant
            database=database_settings.CHROMA_DATABASE,  # database 按项目名隔离，与其他项目分开
        )
        # 按配置映射逐个 get_or_create；cosine 与 DashScope 归一化向量匹配
        for kb_type, name in database_settings.CHROMA_COLLECTIONS.items():
