...
class ExpenseService:
    ...
    async def list(self, page: int = 1, page_size: int = 20) -> ExpensePage:
        ...
        items, total = await self.repo.list_by_user(settings.DEFAULT_USER_ID, page, page_size)
        return ExpensePage(
    ...
    async def get(self, expense_id: int) -> ExpenseResponse:
        ...
        expense = await self.repo.get_by_id(settings.DEFAULT_USER_ID, expense_id)
        if expense is None:
