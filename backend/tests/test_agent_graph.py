...
def test_build_verdict_input_includes_history_reference():
    ...
    reduced, serialized = _build_verdict_input(run_messages, history_reference=history)
...
async def test_verifier_accurate_ends_graph():
    ...
    updates = []
    with (
        patch("app.services.agent.agent_graph._run_verdict", side_effect=fake_run_verdict),
...
async def test_verifier_degrades_to_pass_when_llm_fails():
    ...
    updates = []
    with (
        patch("app.services.agent.agent_graph._run_verdict", side_effect=boom_verdict),
        patch("app.services.agent.agent_graph.create_llm", side_effect=lambda streaming=True, model="", enable_thinking=True, max_tokens=None: agent_llm),
    ):
        async for mode, data in graph.astream(
            {"messages": [HumanMessage(content="hi")], "conv_id": "c1", "user_id": "user-abc", "model": settings.MODEL_OLLAMA},
